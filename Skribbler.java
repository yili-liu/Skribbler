...
public class Skribbler {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Simply enter the word to be drawn or press enter to exit");

        try {
            String s = br.readLine();
            if (s.length() != 0) {
                s = s.replaceAll(" ", "+");
                String res = getHttpResponse(
                        "https://pixabay.com/api/?key=" + API_KEY + "&q=" + s + "&image_type=photo");
    ...
    private static void drawByDots(BufferedImage img) {
        ...
        for (int i = 0; i < toDot.length; i++) {
            for (int j = 0; j < toDot[0].length; j++) {
                if (toDot[i][j] < allRgb[allRgb.length / 30]) {
                    if (!square(X_OFFSET + i * SIDE, Y_OFFSET + j * SIDE, SIDE)) {
                        System.out.println("A robot error has occurred");
                        return;
                    }
                    // for (int k = i * SIDE; k < i * SIDE + 2; k++) {
                    // for (int l = j * SIDE; l < j * SIDE + 2; l++) {
                    // if (!dot(X_OFFSET + k, Y_OFFSET + l)) {
                    // System.out.println("A robot error has occurred");
                    // return;
                    // }
                    // }
                    // }
                } else if (toDot[i][j] < allRgb[allRgb.length / 3]) {
                    if (!dot(X_OFFSET + i * SIDE, Y_OFFSET + j * SIDE)) {
                        System.out.println("A robot error has occurred");
                        return;
                    }
                }
        ...
        System.out.println("Done drawing by dots");
    }

    private static boolean square(int x, int y, int side) {
        try {
            Robot robot = new Robot();
            robot.mouseMove(x, y);
            robot.mousePress(InputEvent.BUTTON1_DOWN_MASK);
            for (int k = x; k < x + side; k++) {
                for (int l = y; l < y + side; l++) {
                    robot.mouseMove(k, l);
                    Thread.sleep(2);
                }
            }
    ...
    private static boolean dot(int x, int y) {
        try {
            Robot robot = new Robot();
            robot.mouseMove(x, y);
            robot.mousePress(InputEvent.BUTTON1_DOWN_MASK);
